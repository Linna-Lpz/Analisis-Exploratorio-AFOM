...
| `analisis_nodos_aislados.R` | Identifica y caracteriza los nodos aislados del grafo MST-kNN: calcula su distribución de cobertura taxonómica original, la tasa de injerto promedio y aplica una prueba de Wilcoxon para determinar si su exclusión introduce sesgo en el análisis funcional. | `mstknn_iter_asignaciones.rds`, `matriz_rf.rds`, bosque en caché | `nodos_aislados_cobertura.csv` con cobertura y porcentaje de injerto de cada nodo aislado |
| `run_pipeline.R` | Orquestador principal del KDD y dependencias. | `config.R`, Scripts | Pipeline completo procesado |

---

## 5. Scripts de Validación y Análisis Complementario (Ejecución Manual)

Los scripts listados a continuación **no forman parte del flujo orquestado por `run_pipeline.R`**. Deben ejecutarse de forma independiente desde RStudio (botón **Source** o `source(...)` en consola), una vez que el pipeline principal haya completado las etapas de las que dependen.

| Script | Cuándo ejecutarlo | Propósito |
|--------|------------------|-----------|
| `13_escalabilidad_subconjuntos.R` | Tras completar los pasos base (matriz RF). | Mide tiempos de ejecución y consumo de RAM para subconjuntos de 1.000 a 15.868 árboles, evaluando la escalabilidad del pipeline. |
| `14_verificacion_PC1_vs_hojas.R` | Tras el PCA (`05d`). | Calcula la correlación entre el score del PC1 y la cobertura taxonómica original, para cuantificar el sesgo de imputación en el espacio de varianza. |
| `15_control_324_arboles_completos.R` | Tras completar el pipeline sobre el corpus completo. | Grupo de control: re-ejecuta clustering y enriquecimiento GO exclusivamente sobre los 324 árboles con 190 especies sin injerto, validando que la señal funcional es intrínseca. |
| `16_analisis_sensibilidad.R` | Tras el injerto filogenético. | Repite el injerto con el segundo mejor medoide y compara la partición resultante mediante ARI para cuantificar la dependencia del andamiaje elegido. |
| `17_comparacion_ari_smith.R` | Tras el análisis de control de 324 árboles. | Calcula el ARI entre la partición obtenida con la distancia de Robinson-Foulds y la basada en la distancia de información de clúster (Smith, 2020) para el grupo de control. |
| `18_calcular_BHI.R` | Tras el enriquecimiento funcional (`09`). | Calcula el Índice de Homogeneidad Biológica (BHI) de la partición final, cuantificando qué proporción de pares en un mismo clúster comparten al menos una categoría GO. |
| `evaluacion_sesgo.R` | Tras el enriquecimiento funcional (`09`). | Contrasta la cobertura taxonómica original entre clústeres con y sin enriquecimiento biológico (Wilcoxon) para descartar sesgo de injerto en el resultado funcional. |
| `analisis_nodos_aislados.R` | Tras completar el clustering MST-kNN iterativo (`07`). | Identifica los árboles excluidos como nodos aislados del grafo MST-kNN, caracteriza su distribución de cobertura original y evalúa si su exclusión introduce sesgo en el análisis funcional. |

> **Nota:** estos scripts leen desde los archivos en caché generados por el pipeline (`Resultados/cache/`) y escriben sus resultados directamente en `Resultados/`. No modifican ningún insumo del pipeline principal.
